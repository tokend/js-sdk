...
gulp.task('test:init_system', ['build:node', 'test:init-istanbul'], function() {
  return gulp.src(["test/test-helper.js", "test/system_init/**/*.js"])
    .pipe(plugins.mocha({
      reporter: ['spec']
    }))
    .pipe(plugins.istanbul.writeReports());
...
gulp.task('test:horizon'/*, ['build:node', 'test:init-istanbul']*/, function() {
    return gulp.src(["test/test-helper.js", "test/integration/horizon_connection_test.js"])
        .pipe(plugins.mocha({
            reporter: ['spec']
        }))
        .pipe(plugins.istanbul.writeReports());
