...
export class CallBuilder {
  ...
  _toCollectionPage(json) {
    ...
    return {
      ...
      next: (keypair) => {
        const url = URI(json._links.next.href).protocol("https");
        if (keypair) {
          var config = this._getRequestConfig(url, keypair);
          return this._sendNormalRequest(url, config)
      ...
      prev: (keypair) => {
        if (keypair) {
          const url = URI(json._links.prev.href).protocol("https");
          var config = this._getRequestConfig(url, keypair);
          return this._sendNormalRequest(url, config)
            .then(r => this._toCollectionPage(r));
        } else {
